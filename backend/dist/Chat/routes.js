...
const AI_1 = require("../database/Request/AI");
const router = (0, express_1.Router)();
router.post("/response_without_auth", [
    ...
    let response = yield (0, AI_1.get_answer_ai_without_auth)(message);
    if (!response.success)
        return res
            .status(404)
...
router.delete("/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (!info_token.success)
            return res.status(403).json(info_token);
        let response = yield (0, Chat_1.delete_chat)(chat_id);
        if (!response.success)
            return res
                .status(404)
                .json({ success: false, message: response.message });
